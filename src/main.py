...
def generate_and_upload_short(youtube: YouTube, tts: TTS, topic: str | None = None) -> None:
    """Generates a Short (optionally from a user-provided topic), then asks
    whether to upload it and cross-post it via Post Bridge.

    Args:
        youtube (YouTube): The initialized YouTube automation instance.
        tts (TTS): The TTS instance used for the voiceover.
        topic (str | None): Optional custom topic. When None, the topic is
            generated by the LLM from the account niche.

    Returns:
        None"""
    youtube.generate_video(tts, topic=topic)
        ...
        if len(cached_accounts) == 0:
            ...
            if selected_account is None:
                ...
                while True:
                    ...
                    if user_input == 1:
                        generate_and_upload_short(youtube, tts)
                    elif user_input == 2:
                        custom_topic = question(" => Enter the topic / video idea for this Short: ").strip()

                        if not custom_topic:
                            warning("No topic entered. Returning to the menu.")
                            continue

                        generate_and_upload_short(youtube, tts, topic=custom_topic)
                    elif user_input == 3:
